...
// GET /api/aspl/seasons/active  — the current active season
...
// POST /api/aspl/seasons  — create a new season (admin only)
...
// PATCH /api/aspl/seasons/:id  — update season fields or status
const updateSeason = async (req, res) => {
  const id = parseInt(req.params.id);
  const {
    name, sport, status,
    max_squad_size, min_squad_size,
    min_bid_price, starting_balance, total_players, registration_open,
  } = req.body;
  ...
  try {
    ...
    const season = await prisma.asplSeason.update({
      ...
      data: {
        ...
        ...(total_players    !== undefined && { total_players:    parseInt(total_players) }),
