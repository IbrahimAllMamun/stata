...
function Skeleton() {
  return (
    <div className="bg-white rounded-2xl overflow-hidden border border-gray-100">
      ...
      <div className="p-5 space-y-3"><div className="h-3 bg-gray-200 rounded animate-pulse w-1/3" /><div className="h-5 bg-gray-200 rounded animate-pulse w-3/4" /><div className="h-4 bg-gray-200 rounded animate-pulse" /></div>
    </div>
  );
}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts.map(post => {
              ...
              return (
                <Link key={post.id} to={`/posts/${post.slug}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all flex flex-col">
                  {img ? (
                    <div className="h-48 overflow-hidden flex-shrink-0"><img src={img} alt={post.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" /></div>
                  ) : (
                    <div className="h-48 bg-gradient-to-br from-[#F39C12]/10 to-[#2F5BEA]/10 flex items-center justify-center flex-shrink-0"><FileText className="w-12 h-12 text-[#2F5BEA]/20" /></div>
                  )}
                  <div className="p-5 flex flex-col flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs text-gray-400">{formatDate(post.created_at)}</span>
                      {post.admin && <><span className="text-gray-300 text-xs">·</span><span className="text-xs text-gray-400">{post.admin.username}</span></>}
                    </div>
                    ...
                    <div className="mt-4 flex items-center gap-1 text-xs font-semibold text-[#2F5BEA] group-hover:text-[#F39C12] transition-colors">Read more <ChevronRight className="w-3.5 h-3.5" /></div>
                  </div>
                </Link>
              );
            })}
          ...
          <div className="text-center py-20 bg-white rounded-2xl border border-gray-100"><FileText className="w-12 h-12 text-gray-200 mx-auto mb-3" /><p className="text-gray-400 font-medium">No posts available yet</p></div>
        )}
      </section>
    </div>
  );
