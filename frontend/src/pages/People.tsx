...
import { Users, Crown, Star, Search, ChevronDown, X, Mail, Phone, Building2, MapPin, Briefcase, Hash } from 'lucide-react';
      ...
      >
        ...
        <div className="px-6 pb-6 space-y-3">
          ...
          {member.alternative_phone && (
            <DetailRow icon={<Phone className="w-4 h-4" />} label="Alt. Phone" value={member.alternative_phone} />
          )}
          {member.job_title && (
            <DetailRow icon={<Briefcase className="w-4 h-4" />} label="Job Title" value={member.job_title} />
          )}
          {member.organisation && (
            <DetailRow icon={<Building2 className="w-4 h-4" />} label="Organisation" value={member.organisation} />
          )}
          {member.organisation_address && (
            <DetailRow icon={<MapPin className="w-4 h-4" />} label="Address" value={member.organisation_address} />
          )}
        </div>
      </div>
    </div>
...
function CommitteeMemberModal({ person, role, year, onClose }: {
  person: any; role: string; year: number; onClose: () => void;
}) {
    ...
    >
      ...
      >
        ...
        <div className="px-6 -mt-12 flex justify-between items-end mb-4">
          {imgSrc ? (
            <img src={imgSrc} alt={person.full_name}
              className="w-24 h-24 rounded-2xl object-cover border-4 border-white shadow-lg" />
          ) : (
            <div className="w-24 h-24 rounded-2xl bg-[#2F5BEA] flex items-center justify-center text-white text-4xl font-bold border-4 border-white shadow-lg">
              {person.full_name.charAt(0).toUpperCase()}
            </div>
          )}
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors mb-1"
          >
            <X className="w-4 h-4 text-gray-500" />
          </button>
        </div>
        ...
        <div className="px-6 pb-6 space-y-3">
          ...
          {person.job_title && (
            <DetailRow icon={<Briefcase className="w-4 h-4" />} label="Job Title" value={person.job_title} />
          )}
          {person.organisation && (
            <DetailRow icon={<Building2 className="w-4 h-4" />} label="Organisation" value={person.organisation} />
          )}
        </div>
      </div>
    </div>
...
// ─── Committee Card (vertical layout) ────────────────────────────────────────

function CommitteeCard({ person, role, year, isCurrent, color, icon, onSelect }: {
  ...
  return (
    <button
      onClick={onSelect}
      className={`w-full bg-white rounded-2xl shadow-sm border overflow-hidden transition-all hover:shadow-lg hover:-translate-y-0.5 cursor-pointer text-left ${isCurrent ? 'border-amber-200' : 'border-gray-100'}`}
    >
      {/* Role badge at top */}
      ...
      <div className="flex flex-col items-center pt-6 pb-5 px-4">
        ...
        {/* Batch */}
        <span className="bg-[#1F2A44] text-white text-xs px-2.5 py-0.5 rounded-full font-medium mb-2">
          Batch {person.batch}
        </span>

        {/* Job info */}
        {person.job_title && (
          <p className="text-xs text-gray-500 text-center truncate w-full">{person.job_title}</p>
        )}
        {person.organisation && (
          <p className="text-xs text-gray-400 text-center truncate w-full">{person.organisation}</p>
        )}
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ...
        setSelectedCommitteePerson(null);
      }
    };
      ...
      <div className="bg-white border-b border-gray-200 sticky top-16 z-30">
        <div className="max-w-7xl mx-auto px-4 flex">
          <button
            ...
            className={`px-6 py-4 text-sm font-semibold border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'members' ? 'border-[#2F5BEA] text-[#2F5BEA]' : 'border-transparent text-gray-500 hover:text-gray-700'
              }`}
          >
          ...
          <button
            ...
            className={`px-6 py-4 text-sm font-semibold border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'committees' ? 'border-[#F39C12] text-[#F39C12]' : 'border-transparent text-gray-500 hover:text-gray-700'
              }`}
          >
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-[#F5F7FA] border-b border-gray-200">
                        <th className="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-10">#</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {filtered.map((member, idx) => (
                        <tr
                          ...
                          className={`cursor-pointer transition-colors ${member.is_president_or_secretary
                        ...
                        >
                          ...
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-2.5">
                              ...
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="font-medium text-[#1F2A44]">{member.full_name}</span>
                                {member.is_president_or_secretary && (
                                  <span className="inline-flex items-center gap-0.5 bg-amber-100 text-amber-700 text-xs px-1.5 py-0.5 rounded-full border border-amber-200">
                          ...
                          <td className="px-5 py-3.5 text-gray-500 text-sm">{member.email}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
