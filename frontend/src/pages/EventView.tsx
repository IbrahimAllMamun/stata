    ...
    <div className="bg-[#F5F7FA] min-h-[calc(100vh-4rem)] py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <article className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          {img ? (
            ...
            <div className="h-40 bg-gradient-to-r from-[#1F2A44] to-[#2F5BEA] flex items-end p-6">
              <span className={`text-xs px-3 py-1.5 rounded-full font-bold ${event.is_upcoming ? 'bg-[#2ECC71] text-white' : 'bg-white/20 text-white'}`}>
                {event.is_upcoming ? '● Upcoming' : 'Past Event'}
              </span>
            </div>
          )}
          ...
          <div className="p-8">
            ...
            {event.description && (
              <div>
                <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-5">About This Event</h2>
        ...
        <div className="mt-6 text-center">
          <Link to="/events" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-[#2F5BEA] font-medium transition-colors">
